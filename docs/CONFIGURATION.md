...
| `--group-by` | `-g` | string | `resource` | Group events by: `resource`, `namespace`, `kind`, `reason` (no effect with `--watch`) |
| `--summary-only` | `-s` | bool | `false` | Show summary statistics only (no effect with `--watch`) |
